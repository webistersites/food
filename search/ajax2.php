...
if($_GET['type'] == 'country'){
	$result = mysql_query("SELECT name FROM sao_francisco.tec_products where category_id = 99 and name LIKE '%".strtoupper($_GET['name_startsWith'])."%'");	
	$data = array();
	while ($row = mysql_fetch_array($result)) {
		array_push($data, $row['name']);	
	}	
...
if($_GET['type'] == 'country_table'){
	...
	$result = mysql_query("SELECT name, cost, id, code FROM sao_francisco.tec_products where category_id = 98 and name LIKE '%".strtoupper($_GET['name_startsWith'])."%'");	
	$data = array();
	while ($row = mysql_fetch_array($result)) {
		$name = $row['name'].'|'.$row['cost'].'|'.$row['id'].'|'.$row['code'].'|'.$row_num;
